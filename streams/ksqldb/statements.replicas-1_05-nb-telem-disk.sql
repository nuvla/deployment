----------------------------------------------------------
--
-- Subscription to NB ram > threshold with email notification
CREATE TABLE SUBS_NB_DISK_ABOVE_EMAIL_T AS
SELECT "resource-id",
...
JOIN SUBS_NB_DISK_ABOVE_EMAIL_T AS subs ON nb_tlm.id = subs."resource-id"
WHERE (ARRAY_CONTAINS(nb_tlm.acl->"owners", subs.owner) OR ARRAY_CONTAINS(nb_tlm.acl->"view-data", subs.owner))
    AND subs.enabled = true
    AND ((nb_tlm.resources_prev_disk1_used_pers <= CAST(subs."value" AS INTEGER)
           AND nb_tlm.resources_disk1_used_pers > CAST(subs."value" AS INTEGER))
        OR
        (nb_tlm.resources_disk1_used_pers < CAST(subs."value" AS INTEGER)
           AND nb_tlm.resources_prev_disk1_used_pers >= CAST(subs."value" AS INTEGER)))
EMIT CHANGES;

----------
--
-- Subscription to NB disk > threshold with slack notification
CREATE TABLE SUBS_NB_DISK_ABOVE_SLACK_T AS
SELECT "resource-id",
    LATEST_BY_OFFSET(notif.method) as method,
    LATEST_BY_OFFSET(notif.destination) as destination,
    LATEST_BY_OFFSET(subs.enabled) AS enabled,
...
SELECT
    ...
    (nb_tlm.resources_prev->disks[1]->"used" > nb_tlm.resources->disks[1]->"used") as recovery
FROM NB_TELEM_RESOURCES_REKYED_S AS nb_tlm
JOIN SUBS_NB_DISK_ABOVE_SLACK_T AS subs ON nb_tlm.id = subs."resource-id"
WHERE (ARRAY_CONTAINS(nb_tlm.acl->"owners", subs.owner) OR ARRAY_CONTAINS(nb_tlm.acl->"view-data", subs.owner))
    AND subs.enabled = true
    AND ((nb_tlm.resources_prev_disk1_used_pers <= CAST(subs."value" AS INTEGER)
           AND nb_tlm.resources_disk1_used_pers > CAST(subs."value" AS INTEGER))
...
----------------------------------------------------------
--
-- Subscription to NB disk < threshold with email notification
CREATE TABLE SUBS_NB_DISK_BELOW_EMAIL_T AS
SELECT "resource-id",
    LATEST_BY_OFFSET(notif.method) as method,
    LATEST_BY_OFFSET(notif.destination) as destination,
    LATEST_BY_OFFSET(subs.enabled) AS enabled,
...
SELECT
    ...
    (nb_tlm.resources_prev->disks[1]->"used" < nb_tlm.resources->disks[1]->"used") as recovery
FROM NB_TELEM_RESOURCES_REKYED_S AS nb_tlm
JOIN SUBS_NB_DISK_BELOW_EMAIL_T AS subs ON nb_tlm.id = subs."resource-id"
WHERE (ARRAY_CONTAINS(nb_tlm.acl->"owners", subs.owner) OR ARRAY_CONTAINS(nb_tlm.acl->"view-data", subs.owner))
    AND subs.enabled = true
    AND ((nb_tlm.resources_prev_disk1_used_pers >= CAST(subs."value" AS INTEGER)
           AND nb_tlm.resources_disk1_used_pers < CAST(subs."value" AS INTEGER))
...
--
-- Subscription to NB disk < threshold with slack notification
CREATE TABLE SUBS_NB_DISK_BELOW_SLACK_T AS
SELECT "resource-id",
    LATEST_BY_OFFSET(notif.method) as method,
    LATEST_BY_OFFSET(notif.destination) as destination,
    LATEST_BY_OFFSET(subs.enabled) AS enabled,
...
SELECT
    ...
    (nb_tlm.resources_prev->disks[1]->"used" < nb_tlm.resources->disks[1]->"used") as recovery
FROM NB_TELEM_RESOURCES_REKYED_S AS nb_tlm
JOIN SUBS_NB_DISK_BELOW_SLACK_T AS subs ON nb_tlm.id = subs."resource-id"
WHERE (ARRAY_CONTAINS(nb_tlm.acl->"owners", subs.owner) OR ARRAY_CONTAINS(nb_tlm.acl->"view-data", subs.owner))
    AND subs.enabled = true
    AND ((nb_tlm.resources_prev_disk1_used_pers >= CAST(subs."value" AS INTEGER)
           AND nb_tlm.resources_disk1_used_pers < CAST(subs."value" AS INTEGER))
