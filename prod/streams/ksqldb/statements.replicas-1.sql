...
WITH (KAFKA_TOPIC='notification-method', PARTITIONS=3, REPLICAS=1, VALUE_FORMAT='JSON');
...
WITH (KAFKA_TOPIC='subscription', PARTITIONS=3, REPLICAS=1, VALUE_FORMAT='JSON');

--
-- subscriptions to event notifications of category 'state'
-- from SUBSCRIPTION_T topic
CREATE TABLE SUBS_EVENT_NOTIF_STATE_T
WITH (PARTITIONS=3, REPLICAS=1, VALUE_FORMAT='JSON')
AS SELECT *
...
WITH (KAFKA_TOPIC='event', PARTITIONS=3, REPLICAS=1, VALUE_FORMAT='JSON');
